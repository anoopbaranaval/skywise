...
class CreateAccountHolder extends Component {
    constructor(props) {
        ...
        this.saveOrUpdateAccountholder = this.saveOrUpdateAccountholder.bind(this);
    }
    ...
    saveOrUpdateAccountholder = (e) => {
        e.preventDefault();
        let accountholder = {firstName: this.state.firstName, lastName: this.state.lastName, emailId: this.state.emailId};
        console.log('accountholder => ' + JSON.stringify(accountholder));

        // step 5
        if(this.state.id === '_add'){
            AccountHolderService.createAccountholder(accountholder).then(res =>{
                this.props.history.push('/accountholders');
            });
        }else{
